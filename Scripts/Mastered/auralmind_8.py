...
def microshift_widen_side(y: np.ndarray, sr: int,
    ...
    """
    NEW Stereo Enhancement: Depth-Adaptive CGMS (Correlation-Guarded MicroShift)
    - Applies a tiny delay ONLY to the SIDE high band (>= hi_split_hz).
    - Depth-adaptive: delay/mix increase on tails, decrease on transients.
    - This preserves attack clarity while adding 3D depth and air.
    ...
    env_s = np.sqrt(sps.convolve(mid * mid, np.ones(win_s) / win_s, mode="same") + 1e-12).astype(np.float32)
    env_l = np.sqrt(sps.convolve(mid * mid, np.ones(win_l) / win_l, mode="same") + 1e-12).astype(np.float32)
...
def get_presets() -> Dict[str, Preset]:
    return {
        ...
        "competitive_trap": Preset(
            ...
            max_eq_db=6.0,
            eq_smooth_hz=95.0,
            movement_amount=0.16,
            width_mid=1.07,
            width_hi=1.30,
            depth_max_cut_db=0.90,
        ...
        "melodic_trap": Preset(
            ...
            name="melodic_trap",
            target_lufs=-12.0,          # Slightly louder than hi-fi but not crushing
            match_strength=0.62,        # Strong tonal matching
            ...
            glow_drive_db=1.2,          # More harmonic warmth for melodic content
            ...
            movement_amount=0.14,       # Dynamic movement
