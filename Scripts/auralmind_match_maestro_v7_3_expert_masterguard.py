...
def true_peak_limiter_v2(
    ...
    low_protect_strength: float = 0.35,
    low_protect_hz: float = 160.0,
    ...
    # Low-band protection: apply LESS gain reduction to lows to preserve punch/density,
    # then rely on ISP correction to guarantee the ceiling.
    lp = float(clamp(low_protect_strength, 0.0, 1.0))
    hz = float(clamp(low_protect_hz, 60.0, 260.0))
    if lp > 1e-6:
        b_lp, a_lp = sps.butter(2, hz / (0.5 * sr), btype="low")
        low = np.zeros_like(y, dtype=np.float32)
        for ch in range(2):
            low[:, ch] = sps.filtfilt(b_lp, a_lp, y[:, ch]).astype(np.float32)
        hi = (y - low).astype(np.float32)

        g_low = (1.0 - (1.0 - g) * lp).astype(np.float32)  # less GR on lows
        y2 = (low * g_low[:, None] + hi * g[:, None]).astype(np.float32)
        g_eff = (np.maximum(g, g_low)).astype(np.float32)  # conservative for stats
    else:
    ...
    if tp_lin > ceiling:
        ...
        y2 = (y2 * corr).astype(np.float32)
        g_eff = (g_eff * corr).astype(np.float32)
        tp_lin = true_peak_estimate(y2, sr, oversample=oversample)

    min_gain = float(np.min(g_eff))
    stats = {
        "min_gain_db": float(lin_to_db(min_gain)),
        "avg_gr_db": float(lin_to_db(float(np.mean(g_eff)) + 1e-12)),
...
def peak_control_chain(
    ...
    y2, st = true_peak_limiter_v2(
        ...
        low_protect_strength=float(getattr(preset, "limiter_low_protect_strength", 0.35)),
        low_protect_hz=float(getattr(preset, "limiter_low_protect_hz", 160.0)),
    )
    st["mode"] = 2.0
    return y2, st
...
class Preset:
    ...
    # Bass integrity guard (protect sub-band punch / avoid low-end flattening)
    enable_bass_guard: bool = True
    bass_guard_lo_hz: float = 20.0
    bass_guard_hi_hz: float = 140.0
    bass_guard_crest_drop_db: float = 1.2   # max allowed drop in sub-band crest factor (dB)
    bass_guard_rms_drop_db: float = 0.8     # max allowed drop in sub-band RMS (dB)

    # Section-aware hook shaping: slightly more low-end density + width ONLY in hooks
    enable_section_aware: bool = True
    hook_low_density: bool = True
    hook_low_density_mix: float = 0.18
    hook_low_density_drive_db: float = 1.4
    hook_low_density_hi_hz: float = 180.0
    movement_hook_only: bool = True

    # Limiter low-band protection (reduces GR applied to lows before ISP correction)
    limiter_low_protect_strength: float = 0.35  # 0..1 (higher => protect lows more)
    limiter_low_protect_hz: float = 160.0
...
def master(target_path: str, out_path: str, preset: Preset,
    ...
    # ---------------------------------------------------------------------
    # Movement + HookLift + Hook Low-Density (section-aware)
    # ---------------------------------------------------------------------
    movement_info: Dict[str, Any] = {"enabled": False}
    hooklift_info: Dict[str, Any] = {"enabled": False}
    hookbass_info: Dict[str, Any] = {"enabled": False}

    # Build a hook/chorus mask once (0..1), used for hook-only processing.
    hook_mask: Optional[np.ndarray] = None
    if bool(getattr(preset, "enable_section_aware", True)):
        hook_mask = build_section_lift_mask(
            y, sr_t,
            percentile=float(getattr(preset, "hooklift_auto_percentile", 75.0)),
        )

    # Movement: optionally hook-only
    if preset.enable_movement:
        if bool(getattr(preset, "movement_hook_only", True)) and hook_mask is not None:
            moved, movement_info = movement_automation(y, sr_t, amount=float(preset.movement_amount))
            mcol = hook_mask[:, None].astype(np.float32)
            y = ((1.0 - mcol) * y + mcol * moved).astype(np.float32)
            movement_info = {**movement_info, "hook_only": True}
        else:
            y, movement_info = movement_automation(y, sr_t, amount=float(preset.movement_amount))
            movement_info = {**movement_info, "hook_only": False}

    # HookLift: width/air only in hooks when section-aware
    if preset.enable_hooklift:
        if bool(getattr(preset, "hooklift_auto", True)) and hook_mask is not None:
            lifted, hinfo = hooklift(y, sr_t, mix=float(preset.hooklift_mix))
            mask_col = hook_mask[:, None]
            y = ((1.0 - mask_col) * y + mask_col * lifted).astype(np.float32)
            hooklift_info = {**hinfo, "auto": True, "auto_percentile": float(getattr(preset, "hooklift_auto_percentile", 75.0))}
        else:
            if hook_mask is not None and bool(getattr(preset, "enable_section_aware", True)):
                lifted, hinfo = hooklift(y, sr_t, mix=float(preset.hooklift_mix))
                mask_col = hook_mask[:, None]
                y = ((1.0 - mask_col) * y + mask_col * lifted).astype(np.float32)
                hooklift_info = {**hinfo, "auto": False, "hook_masked": True}
            else:
                y, hooklift_info = hooklift(y, sr_t, mix=float(preset.hooklift_mix))
                hooklift_info = {**hooklift_info, "auto": False, "hook_masked": False}

    # Hook low-end density: add weight ONLY in hooks (before final loudness/limiting)
    if hook_mask is not None and bool(getattr(preset, "hook_low_density", True)):
        y, hookbass_info = hook_low_density(
            y, sr_t, hook_mask,
            hi_hz=float(getattr(preset, "hook_low_density_hi_hz", 180.0)),
            drive_db=float(getattr(preset, "hook_low_density_drive_db", 1.4)),
            mix=float(getattr(preset, "hook_low_density_mix", 0.18)),
        )

    # Loudness Governor v2 (binary search) + final peak control chain (softclip + TP limiter)# Loudness Governor v2 (binary search) + final peak control chain (softclip + TP limiter)
    ...
    def _render_at(target_lufs: float) -> Tuple[np.ndarray, Dict[str, float]]:
        ...
        # Bass integrity baseline (after gain, before limiting)
        bass_pre = sub_band_metrics(
            y_norm, sr_t,
            lo_hz=float(getattr(preset, 'bass_guard_lo_hz', 20.0)),
            hi_hz=float(getattr(preset, 'bass_guard_hi_hz', 140.0)),
        )
        y_lim, lim_stats = peak_control_chain(y_norm, sr_t, preset)
        bass_post = sub_band_metrics(
            y_lim, sr_t,
            lo_hz=float(getattr(preset, 'bass_guard_lo_hz', 20.0)),
            hi_hz=float(getattr(preset, 'bass_guard_hi_hz', 140.0)),
        )
        lim_stats.update({
            'sub_rms_pre_db': float(bass_pre.get('rms_db', 0.0)),
            'sub_rms_post_db': float(bass_post.get('rms_db', 0.0)),
            'sub_crest_pre_db': float(bass_pre.get('crest_db', 0.0)),
            'sub_crest_post_db': float(bass_post.get('crest_db', 0.0)),
        })
    ...
    for _ in range(steps):
        ...
        # Bass integrity guard: avoid low-end flattening under heavy limiting
        if bool(getattr(preset, 'enable_bass_guard', True)):
            crest_drop = float(getattr(preset, 'bass_guard_crest_drop_db', 1.2))
            rms_drop = float(getattr(preset, 'bass_guard_rms_drop_db', 0.8))
            ok_bass = (
                float(cand_stats.get('sub_crest_post_db', 0.0)) >= float(cand_stats.get('sub_crest_pre_db', 0.0)) - crest_drop
                and float(cand_stats.get('sub_rms_post_db', 0.0)) >= float(cand_stats.get('sub_rms_pre_db', 0.0)) - rms_drop
            )
        else:
            ok_bass = True

        if ok_gr and ok_tp and ok_bass:
    ...
    result = {
        ...
        "sub_rms_pre_db": float(best_stats.get("sub_rms_pre_db", 0.0)) if best_stats is not None else None,
        "sub_rms_post_db": float(best_stats.get("sub_rms_post_db", 0.0)) if best_stats is not None else None,
        "sub_crest_pre_db": float(best_stats.get("sub_crest_pre_db", 0.0)) if best_stats is not None else None,
        "sub_crest_post_db": float(best_stats.get("sub_crest_post_db", 0.0)) if best_stats is not None else None,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # Expert++: Bass integrity + section-aware hook shaping
    p.add_argument("--no-bass-guard", action="store_true", help="Disable bass integrity guard (allows more low-end flattening at high loudness).")
    p.add_argument("--bass-crest-drop-db", type=float, default=None, help="Max allowed sub-band crest drop (dB) through limiting. Default ~1.2.")
    p.add_argument("--bass-rms-drop-db", type=float, default=None, help="Max allowed sub-band RMS drop (dB) through limiting. Default ~0.8.")
    p.add_argument("--bass-guard-hz", type=float, default=None, help="Sub-band high cutoff for bass guard (Hz). Default ~140.")

    p.add_argument("--no-section-aware", action="store_true", help="Disable section-aware processing (hook-only shaping).")
    p.add_argument("--no-hook-bass", action="store_true", help="Disable hook-only low-end density boost.")
    p.add_argument("--hook-bass-mix", type=float, default=None, help="Hook-only bass density wet mix (0..0.45).")
    p.add_argument("--hook-bass-drive-db", type=float, default=None, help="Hook-only bass saturation drive (dB).")
    p.add_argument("--hook-bass-hz", type=float, default=None, help="Hook-only bass lowpass cutoff (Hz).")
    p.add_argument("--movement-global", action="store_true", help="Force movement automation across the whole track (not hook-only).")

    p.add_argument("--limiter-low-protect", type=float, default=None, help="Limiter low-band protection strength 0..1 (higher preserves bass punch).")
    p.add_argument("--limiter-low-hz", type=float, default=None, help="Limiter low-band protection cutoff (Hz).")
...
def main():
    ...
    # Bass integrity + section-aware overrides
    if args.no_bass_guard:
        updates["enable_bass_guard"] = False
    if args.bass_crest_drop_db is not None:
        updates["bass_guard_crest_drop_db"] = float(args.bass_crest_drop_db)
    if args.bass_rms_drop_db is not None:
        updates["bass_guard_rms_drop_db"] = float(args.bass_rms_drop_db)
    if args.bass_guard_hz is not None:
        updates["bass_guard_hi_hz"] = float(args.bass_guard_hz)

    if args.no_section_aware:
        updates["enable_section_aware"] = False
    if args.no_hook_bass:
        updates["hook_low_density"] = False
    if args.hook_bass_mix is not None:
        updates["hook_low_density_mix"] = float(args.hook_bass_mix)
    if args.hook_bass_drive_db is not None:
        updates["hook_low_density_drive_db"] = float(args.hook_bass_drive_db)
    if args.hook_bass_hz is not None:
        updates["hook_low_density_hi_hz"] = float(args.hook_bass_hz)
    if args.movement_global:
        updates["movement_hook_only"] = False

    if args.limiter_low_protect is not None:
        updates["limiter_low_protect_strength"] = float(args.limiter_low_protect)
    if args.limiter_low_hz is not None:
        updates["limiter_low_protect_hz"] = float(args.limiter_low_hz)
